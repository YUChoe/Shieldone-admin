...
ShieldOne의 모든 기능은 GUI 를 통해 설정이 가능합니다.
...
.. index:: 관리화면, 대시보드, 상태화면, main manu

ShieldOne 관리화면 구성
===========================

보안프로토콜을 통해 ShieldOne UTM의 관리자/운영자 계정으로 접속하면 아래와 같은 대시보드(상태화면)이 보여
집니다. ShieldOne UTM의 관리 메뉴는 크게 11가지로 나뉘어져 있습니다.
...
ShieldOne UTM 시스템의 현재 상태를 한 화면에 모니터링 하는 기능입니다.
...
1. 시스템의 세부적인 내용을 보여줍니다. 내용은 자동으로 갱신 됩니다.
    ...
    | 종류                | 기능설명                                     |
    +=====================+==============================================+
    | Hostname            | 호스트 이름(Serial)                          |
    +---------------------+----------------------------------------------+
    | License Expire      | 라이선스의 만료 날짜                         |
    +---------------------+----------------------------------------------+
    | Uptime              | ShieldOne UTM 장비의 구동 시간               |
    +---------------------+----------------------------------------------+
    | Localtime           | ShieldOne UTM 장비 내부의 시간               |
    +---------------------+----------------------------------------------+
    | Current Session(s)  | 현재 내부 네트워크의 세션 수                 |
    +---------------------+----------------------------------------------+
    | SSL Accounts        | 현재 생성된 계정 수 / Maximum 계정 수        |
    +---------------------+----------------------------------------------+
    | 기타                | 라이선스 종류와 작동 모드에 따라 관련 정보   |
    |                     | 클릭을 통해 ON/OFF                           |
    +---------------------+----------------------------------------------+
...
3. 각 기능 모듈의 작동여부를 표시 합니다. 클릭을 통해 해당기능을 ON/OFF 할 수 있습니다.

4. 이벤트 로그 : 최신 방화벽과 IPS 로그를 보여줍니다. 30초 간격으로 자동 갱신 됩니다.
